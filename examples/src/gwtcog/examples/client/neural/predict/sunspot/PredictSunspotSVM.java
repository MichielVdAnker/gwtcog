 ...
 */
package gwtcog.examples.client.neural.predict.sunspot;
...
import gwtcog.core.ml.svm.SVM;
import gwtcog.core.ml.svm.training.SVMTrain;
import gwtcog.core.util.EngineArray;
import gwtcog.core.util.arrayutil.NormalizeArray;
import gwtcog.core.util.arrayutil.TemporalWindowArray;
 ...
 */
public class PredictSunspotSVM implements RunnableDemo {
	...
	public void run(Panel parent) {	
		...
		//show we've started
		panel.add(new Label("PredictSunspotSVM Demo Started"));
	...
	public void predict(SVM network)
	{
		...
		panel.add(new Label("Year\tActual\tPredict\tClosed Loop Predict"));
		...
		for(int year=EVALUATE_START;year<EVALUATE_END;year++)
		{
			...
			// display
			panel.add(new Label((STARTING_YEAR+year)
					+"\t"+normalizedSunspots[year]//f.format(this.normalizedSunspots[year])
							+"\t"+prediction//f.format(prediction)
							+"\t"+closedLoopPrediction//f.format(closedLoopPrediction)
					));
