...
import gwtcog.core.ml.MLStateSequence;
...
import gwtcog.core.ml.hmm.alog.ViterbiCalculator;
...
//	public HiddenMarkovModel clone() throws CloneNotSupportedException {
		...
		hmm.pi = CloneTool.clone(this.pi);//this.pi.clone();
		hmm.transitionProbability = CloneTool.clone(this.transitionProbability);//this.transitionProbability.clone();

		for (int i = 0; i < this.transitionProbability.length; i++) {
			hmm.transitionProbability[i] = CloneTool.clone(this.transitionProbability[i]);//this.transitionProbability[i].clone();
		}
